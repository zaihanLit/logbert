...
class Trainer:
    def __init__(self, options):
        ...
        if self.sample == 'sliding_window':
            ...
            logkeys = split_features(self.data_dir + "train",
                                            self.train_ratio,
                                            min_len=self.min_len)

            train_logkeys, valid_logkeys = train_test_split(logkeys, test_size=self.valid_ratio)

            print("Loading vocab")
            with open(self.vocab_path, 'rb') as f:
                vocab = pickle.load(f)

            train_logs, train_labels = sliding_window((train_logkeys),
                                                      vocab=vocab,
                                                      window_size=self.window_size,
                                                      )

            val_logs, val_labels = sliding_window((valid_logkeys),
                                                  vocab=vocab,
                                                  window_size=self.window_size,
                                                  )
            del train_logkeys
    ...
    def start_train(self):
        ...
        Utils.plot_train_valid_loss(self.model_dir)
